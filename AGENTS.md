...
- The root-owned Git-tracked plan is authoritative. Codex requires a matching native Goal for every implementation. Automatic Goal creation follows standing or per-task user/host authorization without repeated consent; it never overrides host permissions or an unrelated active Goal. If the host cannot create or verify the Goal, report the concrete blocker and stop tracked dispatch.
