...
For target-project code, the lifecycle selects the simplest maintainable complete design for verified current needs, reuses existing mechanisms, and requires concrete justification for significant new complexity. Before freeze, the implementer and independent reviewer perform the canonical [simplicity and architecture review](../plugins/ant/skills/implementation-orchestrator/references/lifecycle.md#simplicity-and-architecture), including safe removal or combination of unnecessary layers, duplicate state, unjustified genericity, and tests without unique coverage.
...
A user's explicit instruction takes precedence over a skill-owned workflow preference and can change the scoped plan or endpoint; the [shared lifecycle](../plugins/ant/skills/implementation-orchestrator/references/lifecycle.md) records the impacts and any pending evidence within higher-priority safety and permission bounds.

The plan and any established Goal describe the same measurable outcome. Goal handling follows the [shared lifecycle](../plugins/ant/skills/implementation-orchestrator/references/lifecycle.md) and active host adapter. On Codex, Goals are handled only when explicitly requested or already in use; missing or unavailable optional Goal support never blocks otherwise-authorized work under the plan, while an explicitly requested Goal remains pending until established and verified. A Goal never grants permission or replaces the plan, and unrelated Goals remain protected. Cross-thread handoffs preserve consent provenance, scope bounds, the exact follow-up, and any proposed delta; the receiving agent verifies that context under the [blocker adjudication policy](../plugins/ant/skills/implementation-orchestrator/references/lifecycle.md#blocker-adjudication-and-scope).

The shared [blocker adjudication policy](../plugins/ant/skills/implementation-orchestrator/references/lifecycle.md#blocker-adjudication-and-scope) distinguishes ordinary findings, capability failures or timeouts, explicit safety denials, and notices that offer a permitted reconsideration path. Reports preserve the exact operation, source, observed reason or notice, affected scope, and any skill owner rule, separating observation from interpretation. A denied operation remains subject to the host's refusal; an explicitly permitted recovery path may be followed only within higher-priority rules and required authority, and unrelated authorized work continues independently.
