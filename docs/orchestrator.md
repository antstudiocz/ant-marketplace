...
5. Run targeted checks after coherent phases, updating the Plan at phase/wave transitions; perform independent strong review after final edits, repairs/re-review, then one final broad suite with matching Plan evidence.
...
| Codex root | `gpt-5.6-sol` + High | Verify from authoritative host/session/task metadata or explicit current host selection. Model self-identification is not evidence. If exact route cannot be verified, fail before tracked-writer dispatch. |
...
Fail closed when the exact mandatory root route, child route, effort, isolation, or Codex `fork_turns="none"` cannot be enforced. Correct and discard a child result when observable runtime metadata exposes a mismatch before re-dispatching.
