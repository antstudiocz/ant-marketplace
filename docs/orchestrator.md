...
Interactive smoke is optional unless explicitly requested or required by acceptance criteria. Classify the requested endpoint as implementation-only, draft PR/MR create/update, or explicitly merge-ready; the shared [lifecycle](../plugins/ant/skills/implementation-orchestrator/references/lifecycle.md) owns handoff provenance, endpoint completion, and final reporting. Provider delivery follows the explicit Create/update authority in [`merge-request`](../plugins/ant/skills/merge-request/SKILL.md); merge, release, and deployment remain separate authority. Completion uses the lifecycle's achieved/pending/blocked outcome with its evidence, gaps, and next permitted action. Deployment requires explicit authorization and verification.
