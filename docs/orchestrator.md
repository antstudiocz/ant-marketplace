...
The shared [blocker adjudication policy](../plugins/ant/skills/implementation-orchestrator/references/lifecycle.md#blocker-adjudication-and-scope) distinguishes reviewer findings and child uncertainty from capability failures or timeouts and from explicit host or tool safety denials. Root may resolve ordinary findings with evidence or an in-scope alternative, while a denied operation remains subject to the host's refusal and does not prohibit unrelated authorized work.
