...
Treat 404 metadata as part of the route design, not an afterthought.

Rules:

- A branded or localized 404 should have explicit metadata.
- 404 metadata should be `noindex`.
- `follow: true` is usually the clean default unless the site has a reason to block link following.
- Do not emit a canonical URL for a 404 page.
- Do not rely on a whole-environment `noindex` to substitute for correct 404 metadata.

Keep 404 metadata copy separate from visible 404 body copy when they serve different purposes:

- visible copy helps the user recover,
- metadata copy should stay short and explicit.
