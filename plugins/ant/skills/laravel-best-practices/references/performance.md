...
→ **Load `references/caching.md`** for cache method selection, invalidation strategy, and layer recommendations. That reference covers `Cache::memo()`, `Cache::flexible()`, cache tags, HTTP headers, and the full cache layer stack.
...
- [Items to review with `references/caching.md`]
```
