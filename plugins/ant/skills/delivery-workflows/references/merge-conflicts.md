...
This is a precise assignment brief, not a forked conversation. Use only this file path, repository path, conflict inventory, and instructions as your operating context. The subagent must be spawned with a fresh-task / no-history / no-fork mode. If it can see prior chat that was not included in this assignment, it must ignore it and report `Delegation violation: inherited conversation history`. Do not infer merge intent from missing chat history; derive intent from the conflicting code, dependencies, and tests.
