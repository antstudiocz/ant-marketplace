...
4. **Run startup contract** - before detailed planning, ask and record all blocking planning, cadence, phase approval, commit, delivery, MR, and pipeline preferences that change scope, risk, autonomy, or validation. Do not cap the number of required questions; group them clearly when there are many.
...
- **Approval envelope gate:** the user may approve a multi-step workstream only when the assistant states the full authorized envelope, including phases, stop conditions, verification requirements, milestone commit policy, and delivery boundaries. Inside that envelope, agents may continue automatically after verified checkpoints only when the recorded phase approval policy allows it.
