...
- Child agents report questions and blockers to their parent, not to the user. The root resolves repository-discoverable facts and implementation choices already covered by the stable plan; it asks the user only when a material scope/risk change, uncovered destructive or external action, or genuinely missing product decision requires new authority.
