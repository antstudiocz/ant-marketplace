...
5. Dispatch one integration owner with a complete [routing capsule](references/lifecycle.md#dispatch-capsule). Add disjoint workers only when ownership is clear and the active adapter permits them. Every child is fresh, isolated, explicitly routed, and capped at High; the plan directory is read-only to children.
...
7. Apply the selected verification cadence, then verify the selected outcome evidence and realistic states. Complete the mandatory [simplicity and architecture review](references/lifecycle.md#simplicity-and-architecture) before freeze, repair material in-scope findings, and refresh affected evidence. Final candidate review findings or subsequent tracked mutations require affected re-review and refreshed evidence. Freeze one exact candidate and verify one risk-appropriate broad gate.
